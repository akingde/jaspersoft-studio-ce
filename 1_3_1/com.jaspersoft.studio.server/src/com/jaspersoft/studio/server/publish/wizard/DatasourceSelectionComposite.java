 ...
 * to a report unit when created in the remote JasperServer repository.
 * 
...
public class DatasourceSelectionComposite extends Composite {
	...
	/*
	 * Enables (and resets) the internal status of the widget
	 * depending on the type of datasource we are creating/modifying.
	 */
	...
	/*
	 * Performs the selection of a local datasource.
	 * Shows a dialog where the user can choose the right one. 
	 */
	private void selectLocalDatasource() {
		...
		if (ref == null) {
			...
			MResource r = ResourceFactory.getResource(null,
					SelectorDatasource.cloneResource(ref), -1);
			ResourceWizard wizard = new ResourceWizard(parent, r);
			WizardDialog dialog = new WizardDialog(shell, wizard);
			dialog.create();
			if (dialog.open() != Dialog.OK)
				return;
			SelectorDatasource.copyDSFields(r.getValue(), ref);
		}
		textLocalDS.setText(Misc.nvl(ref.getUriString()));
	}

	/*
	 * Performs the selection of a datasource from a remote repository.
	 * Shows a dialog where the user can choose the right one.
	 */
	...
	/*
	 * Remove a previous existing datasource from the MResource
	 * instance specified.
	 */
