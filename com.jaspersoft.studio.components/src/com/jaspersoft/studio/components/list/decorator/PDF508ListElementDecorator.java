...
/**
 * Show an additional action on report and table to set the tag of the list element
 * 
 * @author Orlandin Marco
 *
 */
public class PDF508ListElementDecorator extends TextElementDecorator{

	/**
	 * Decorator used for this tag
	 */
	private PDFListDecorator decorator = new PDFListDecorator();
	...
	public void fillContextMenu(ActionRegistry registry, IMenuManager menu, IStructuredSelection sel) {
		...
		MenuManager submenuTable = new MenuManager(Messages.PDF508ListElementDecorator_actionName);
	...
	public void buildContextMenu(ActionRegistry registry, EditPartViewer viewer, IMenuManager menu) {
		...
		if (sel.getFirstElement() instanceof EditPart) {
			...
			if (!(ep.getModel() instanceof MList))
				return;
		}
