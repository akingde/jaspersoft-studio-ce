	...
	public void refresh() {
		...
		if (element != null) {
			foreButton.setData((RGB) element
					.getPropertyValue(JRBaseChart.PROPERTY_SUBTITLE_COLOR));
			btnFont.setData(element, (MFont) element
					.getPropertyValue(JRDesignChart.PROPERTY_SUBTITLE_FONT));
			expr.setData((JRDesignExpression) element
					.getPropertyValue(JRDesignChart.PROPERTY_SUBTITLE_EXPRESSION));
		}
		isRefreshing = false;
	}
