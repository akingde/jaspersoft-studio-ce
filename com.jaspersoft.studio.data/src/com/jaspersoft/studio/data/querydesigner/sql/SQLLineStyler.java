...
public class SQLLineStyler implements LineStyleListener {
	...
	private void initializeColors() {
		...
		colorsMap.put(SQLTokensType.JRPARAMETER,ResourceManager.getColor(255, 0, 0));
	...
	public void lineGetStyle(LineStyleEvent event) {
	    ...
	    if (inBlockComment(event.lineOffset, event.lineOffset+event.lineText.length())) {
	          styles.add(new StyleRange(event.lineOffset, event.lineText.length(), 
	        		  getColor(SQLTokensType.COMMENT), null));
	    ...
	    while (token != SQLTokensType.EOF) {
	      if (token == SQLTokensType.OTHER) {
	        ...
	        if ((!color.equals(defaultFgColor)) || (token == SQLTokensType.KEYWORD) || (token == SQLTokensType.SYMBOL)) {
	          StyleRange style = new StyleRange(sqlScanner.getStartOffset()
	              + event.lineOffset, sqlScanner.getLength(), color,
	              null);
	...
	/**
	 * Parses the block comments up front since block comments can go across lines.
	 * 
