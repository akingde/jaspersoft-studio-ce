...
public final class ImageConverter extends ElementConverter {
		...
		if (cacheRenderer == null || System.currentTimeMillis() - last > 1000) {
			Job job = new Job("load map") {
				protected IStatus run(IProgressMonitor monitor) {
					Renderable r = getRenderable(reportConverter.getJasperReportsContext(), image, printImage);
					cache.put(element, r);
	...
	private Renderable getRenderable(JasperReportsContext jasperReportsContext, JRImage imageElement,
		...
		if (location != null) {
			try {
				r = RenderableUtil.getInstance(jasperReportsContext).getRenderable(location);
			} catch (JRException e) {
