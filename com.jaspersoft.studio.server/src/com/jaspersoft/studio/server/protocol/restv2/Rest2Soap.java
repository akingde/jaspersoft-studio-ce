...
public class Rest2Soap {
	...
	public static ResourceDescriptor getRD(ARestV2Connection rc,
		...
		DiffFields.setSoapValue(rd, DiffFields.PERMISSIONMASK,
				Misc.nvl(cr.getPermissionMask(), 0));
	...
	private static void getAdhocDataView(ARestV2Connection rc,
		...
		if (cr.getDataSource() != null)
			rd.getChildren().add(
					getRDContainer(rc, (ClientResource<?>) cr.getDataSource()));
	}
	...
	private static void getReportUnit(ARestV2Connection rc,
		...
		if (cr.getFiles() != null)
			for (ClientReferenceableFile crf : cr.getFiles().values())
				rd.getChildren().add(
						getRDContainer(rc, (ClientResource<?>) crf));
