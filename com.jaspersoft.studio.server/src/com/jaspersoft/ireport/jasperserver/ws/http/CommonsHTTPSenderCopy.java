 ...
 *         not recognizing it
 */
public class CommonsHTTPSenderCopy extends BasicHandler {
	private static final long serialVersionUID = 8881188152022966420L;

	/** Field log */
	protected static Log log = LogFactory.getLog(CommonsHTTPSenderCopy.class
			.getName());
	...
	public CommonsHTTPSenderCopy() {
		initialize();
	}

	protected void initialize() {

		// httpChunkStream = !IReportManager.getPreferences().getBoolean(
		// "jasperserver.preventChunkedRequests", true);

		// IReportManager.getPreferences().addPreferenceChangeListener(
		// new PreferenceChangeListener() {
		//
		// public void preferenceChange(PreferenceChangeEvent pce) {
		// CommonsHTTPSender.this.httpChunkStream = !IReportManager
		// .getPreferences().getBoolean(
		// "jasperserver.preventChunkedRequests",
		// true);
		// }
		// });
		...
		params.setDefaultMaxConnectionsPerHost(clientProperties
				.getMaximumConnectionsPerHost());
		params.setMaxTotalConnections(clientProperties
		...
		if (clientProperties.getDefaultConnectionTimeout() > 0)
			params.setConnectionTimeout(clientProperties
					.getDefaultConnectionTimeout());
		if (clientProperties.getDefaultSoTimeout() > 0)
			params.setSoTimeout(clientProperties.getDefaultSoTimeout());
	...
	public void invoke(MessageContext msgContext) throws AxisFault {
		...
		try {
			...
			httpClient.getParams().setConnectionManagerTimeout(
					clientProperties.getConnectionPoolTimeout());
	...
	public void handleCookie(String cookieName, String cookie,
		...
		if (cookies.size() == 1) {
			...
			msgContext.setProperty(cookieName,
					cookies.toArray(new String[cookies.size()]));
		}
	}
	...
	/**
	 * This method has been modified from the original one provided by Axis in
	 * order to use the NetBeans proxy settings...
	 * 
	 * @param client
	 * @param context
	 * @param targetURL
	...
	protected HostConfiguration getHostConfiguration(HttpClient client,
		...
		// boolean hostInNonProxyList = proxySettings
		// .isHostInNonProxyList(targetURL.getHost());
		...
		// if (!hostInNonProxyList && proxySettings.getHttpHost().length() != 0
		// && proxySettings.getHttpPort() > 0) {
		// if (proxySettings.getUsername() != null
		// && proxySettings.getUsername().length() != 0) {
		...
		// if (domainIndex > 0) {
		// String domain = proxySettings.getUsername().substring(0,
		// domainIndex);
		// if (proxySettings.getUsername().length() > domainIndex + 1) {
		// String user = proxySettings.getUsername().substring(
		// domainIndex + 1);
	...
	private void addContextInfo(HttpMethodBase method, HttpClient httpClient,
		...
		if (msgContext.getTimeout() != 0) {
			...
			httpClient.getHttpConnectionManager().getParams()
					.setSoTimeout(msgContext.getTimeout());
		...
		if (userID != null) {
			...
			if (domainIndex > 0) {
				...
				if (userID.length() > domainIndex + 1) {
					...
					proxyCred = new NTCredentials(user, passwd,
