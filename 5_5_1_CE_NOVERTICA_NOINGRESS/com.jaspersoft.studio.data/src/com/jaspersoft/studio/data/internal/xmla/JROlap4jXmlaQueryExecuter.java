...
import com.jaspersoft.studio.data.internal.olap.JROlapDataSource;



/**
 * @author swood
 * @version $Id: JRXmlaQueryExecuter.java 5576 2012-08-16 09:04:00Z lucianc $
 */
public class JROlap4jXmlaQueryExecuter extends JRAbstractQueryExecuter
{
	...
	private static final Log log = LogFactory.getLog(JROlap4jXmlaQueryExecuter.class);
	 ...
	 * 
	 */
	public JROlap4jXmlaQueryExecuter(
	 ...
	 */
	public JROlap4jXmlaQueryExecuter(JRDataset dataset, Map<String, ? extends JRValueParameter> parametersMap)
	{
	...
	protected void parseCellDataElement(CellSet result) throws JRException
	{
		...
		for (Position axis_1 : result.getAxes().get(Axis.ROWS.axisOrdinal())
				.getPositions()) {
			for (Position axis_0 : result.getAxes()
					.get(Axis.COLUMNS.axisOrdinal()).getPositions()) {
	...
	protected void handlePosition(JRXmlaResultAxis axis, CellSetAxis itAxisElement, Position p)
	{
		...
		while (it.hasNext()) {
			...
			String caption = null;
			try {
				Method captionMethod = m.getClass().getDeclaredMethod("getCaption", new Class[] { Locale.class });
				// This is a olap4j 0.9.7 Member with a getCaption(Locale) method
				try {
					captionMethod.setAccessible(true);
				...
				try {
					...
					Method captionMethod = m.getClass().getDeclaredMethod("getCaption");
					captionMethod.setAccessible(true);
