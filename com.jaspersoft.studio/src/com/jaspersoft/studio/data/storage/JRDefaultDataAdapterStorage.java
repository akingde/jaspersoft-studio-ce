 ...
 * net.sf.jasperreports.data.adapter of a report. It will return nothing
 * if the property is not defined or doesn't point a valid file.
 * 
...
public class JRDefaultDataAdapterStorage extends ADataAdapterStorage {
	...
	/**
	 * Map of the resolvers for each location, each location has its own resolver that handle
	 * also caching and reloading 
	 */
	 ...
	 * 
	 * @return a data adapter descriptor if the property for the default adapter
	 * is available on the dataset and can be resolved, otherwise null
	 */
	@Override
	 ...
	 * 
	 * @param dataset the dataset where the property is searched 
	 * @return a data adapter descriptor if the property for the default adapter
	 * is available on the dataset and can be resolved, otherwise null
	 */
	@Override
	 ...
	 * 
	 * @param location the location of the adapter, can be an url
	 ...
	 * 
	 * @param dataset the location is read from here, if null is search on the main dataset
	 * @return the data adapter if the location was resolved otherwise
	 * null
	 */
	public DataAdapterDescriptor getDefaultJRDataAdapter(JRDesignDataset dataset){
		...
		if (dataset != null) {
			...
			location = design.getMainDataset().getPropertiesMap().getProperty(DataAdapterParameterContributorFactory.PROPERTY_DATA_ADAPTER_LOCATION);
		}
	...
	/**
	 * Return URL for the passed data adapter if it is one adapter provided
	 * by this class, otherwise null
	 */
	@Override
