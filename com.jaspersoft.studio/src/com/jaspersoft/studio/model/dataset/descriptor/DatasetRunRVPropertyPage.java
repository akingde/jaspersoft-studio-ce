...
public class DatasetRunRVPropertyPage extends RVPropertyPage {
	...
	public DatasetRunRVPropertyPage(MDatasetRun datasetRun) {
		super("datasetrunproperties", datasetRun != null ? datasetRun
				.getJasperDesign() : null);
		if (datasetRun != null) {
	 ...
	 */
	public DatasetRunRVPropertyPage(JRDesignDatasetRun datasetRun,
			JasperDesign design) {
	...
	protected void openAddDialog() {
		...
		EditableInputReturnValueDialog inputDialog = new EditableInputReturnValueDialog(
				getShell(), container, getToVariablesNames(),
	...
	protected void openEditDialog(ReturnValueContainer edited) {
		...
		String[] toVariables = getVariablesPlusElement(getToVariablesNames(),
				edited.getToVariable());
		String[] fromVariables = getVariablesPlusElement(
				getFromVariablesNames(), edited.getFromVariable());
		HashSet<String> invalidTo = new HashSet<String>();
		HashSet<String> invalidFrom = new HashSet<String>();
		for (ReturnValueContainer definedValues : getValue()) {
		...
		EditableInputReturnValueDialog inputDialog = new EditableInputReturnValueDialog(
				getShell(), result, toVariables, fromVariables, invalidTo,
				invalidFrom);
		if (inputDialog.open() == Dialog.OK) {
	...
	/*
	 * @Override protected void addElemenet(ReturnValueContainer container) {
	 * super.addElemenet(container);
	 * 
	 * //Check the from variable String fromVariable =
	 * container.getFromVariable(); String[] fromVariables =
	 * getFromVariablesNames(); if (!ArrayUtils.contains(fromVariables,
	 * fromVariable) && datasetRun != null){ //Need to add the variable to the
	 * subdataset String datasetName = datasetRun.getDatasetName();
	 * JRDesignDataset dataset =
	 * (JRDesignDataset)design.getDatasetMap().get(datasetName);
	 * JRDesignVariable newVariable = new JRDesignVariable();
	 * newVariable.setName(fromVariable); try {
	 * dataset.addVariable(newVariable); } catch (JRException e) {
	 * e.printStackTrace(); } }
	 * 
	 * //Check the to variable String toVariable = container.getToVariable();
	 * String[] toVariables = getToVariablesNames(); if
	 * (!ArrayUtils.contains(toVariables, toVariable) && design != null){ //Need
	 * to add the variable to the main dataset JRDesignDataset dataset =
	 * (JRDesignDataset)design.getMainDataset(); JRDesignVariable newVariable =
	 * new JRDesignVariable(); newVariable.setName(toVariable); try {
	 * dataset.addVariable(newVariable); } catch (JRException e) {
	 * e.printStackTrace(); } } }
	 * 
	 * @Override protected void editElement(ReturnValueContainer edited,
	 * ReturnValueContainer newValue) { super.editElement(edited, newValue);
	 * 
	 * //Check the from variable String fromVariable =
	 * newValue.getFromVariable(); String[] fromVariables =
	 * getVariablesPlusElement(getFromVariablesNames(),
	 * edited.getFromVariable()); if (!ArrayUtils.contains(fromVariables,
	 * fromVariable) && datasetRun != null){ //Need to add the variable to the
	 * subdataset String datasetName = datasetRun.getDatasetName();
	 * JRDesignDataset dataset =
	 * (JRDesignDataset)design.getDatasetMap().get(datasetName);
	 ...
	 * e.printStackTrace(); } }
	 * 
	 * //Check the to variable String toVariable = newValue.getToVariable();
	 * String[] toVariables = getVariablesPlusElement(getToVariablesNames(),
	 * edited.getToVariable()); if (!ArrayUtils.contains(toVariables,
	 * toVariable) && design != null){ //Need to add the variable to the main
	 * dataset JRDesignDataset dataset =
	 * (JRDesignDataset)design.getMainDataset(); JRDesignVariable newVariable =
	 * new JRDesignVariable(); newVariable.setName(toVariable); try {
	 * dataset.addVariable(newVariable); } catch (JRException e) {
	 * e.printStackTrace(); } } }
	...
	/**
	 * Update the status of the buttons enabling or disabling them if there are
	 * input errors
	 */
	protected void updateButtonsStatus() {
		if (design != null) {
			JRDesignDataset mainDataset = (JRDesignDataset) design
					.getMainDataset();
			String datasetName = datasetRun.getDatasetName();
			JRDesignDataset subDataset = (JRDesignDataset) design
					.getDatasetMap().get(datasetName);
	...
	/**
	 * Change the dataset run handled by this page. When this method is called
	 * the table showing the return values it is reinitialized with the
	 * information from the new dataset run
	 * 
	 * @param datasetRun
	 *            the jr dataset run to edit
	 * @param design
	...
	/**
	 * Take the information on the return values from the current dataset run
	 * and show them inside the table
	 */
	private void updateDatasetRunReturnValues() {
		if (datasetRun != null) {
			List<ReturnValue> returnValues = datasetRun.getReturnValuesList();
			setValue(ReturnValueContainer
					.convertFromDatasetRunReturn(returnValues));
		}
	}

	/**
	 * Take the information on the return values from the table and save them
	 * inside the dataset run
	 */
	public void saveValuesIntoDataset() {
		if (datasetRun != null) {
			List<ReturnValueContainer> actaulValues = getValue();
			List<ReturnValue> newValues = ReturnValueContainer
					.convertToDatasetRun(actaulValues);
			List<ReturnValue> oldValues = new ArrayList<ReturnValue>(
					datasetRun.getReturnValuesList());
			// The element dosen't allow to swap the list, must remove the old
			// element
