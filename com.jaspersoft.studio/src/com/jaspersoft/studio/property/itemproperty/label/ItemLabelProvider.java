...
public class ItemLabelProvider extends ColumnLabelProvider implements
		ITableLabelProvider {
	...
	public String getText(Object element) {
		if (element instanceof Collection<?>) {
			...
			return Misc.nvl(((JRDatasetRun) element).getDatasetName(),
					Messages.ItemLabelProvider_3);
			...
			if (!Misc.isNullOrEmpty(item.getProperties())) {
				...
				for (ItemPropertyDescription<?> ipd : descriptor
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		} else if (element instanceof StandardItem
				&& descriptor.getItemPropertyDescriptors().length > columnIndex) {
			StandardItem item = (StandardItem) element;
			if (!Misc.isNullOrEmpty(item.getProperties())) {
				ItemPropertyDescription<?> showProperty = descriptor
						.getItemPropertyDescriptors()[columnIndex];
				for (ItemProperty ip : item.getProperties()) {
	...
	public String getToolTipText(Object element) {
		if (element instanceof JRElementDataset) {
			...
			if (ed.getResetTypeValue() == ResetTypeEnum.GROUP
					&& ed.getResetGroup() != null)
				str += ed.getResetGroup().getName();
			str += "\n"; //$NON-NLS-1$
			str += Messages.ItemLabelProvider_13 + ed.getIncrementTypeValue();
			if (ed.getIncrementTypeValue() == IncrementTypeEnum.GROUP
					&& ed.getIncrementGroup() != null)
				str += ed.getIncrementGroup().getName();
			str += "\n"; //$NON-NLS-1$
			if (ed.getIncrementWhenExpression() != null)
				str += Messages.ItemLabelProvider_15
						+ ed.getIncrementWhenExpression().getText();
