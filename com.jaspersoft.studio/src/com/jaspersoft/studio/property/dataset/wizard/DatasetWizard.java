...
/**
 * A Wizard to create an Dataset. The dataset can be get from the wizard using {@link #getDataset() getDataset}
 * which returns a {@link com.jaspersoft.studio.model.dataset.MDataset}.
 * 
 * 
 * @author gtoffoli
 *
...
public class DatasetWizard extends JSSWizard {
	...
	/**
	 * This is the method that builds a JRDesignDataset from the settings get during the wizard steps.
	 * This method returns a new dataset instance each time, there is no cached object.
	 *
	 * @return
	 * @throws JRException 
	 */
	...
	/**
	 * This method force the user to go next and not finish if the selection
	 * in the first step is not an empty dataset...
	 */
	@Override
	public boolean canFinish() {
		...
		{
			if (step1.isPageComplete() &&
					step1.isEmptyDataset())
			{
				return true;
			}
			return false;
	...
	/**
	 * Get the dataset, and add the command to add the dataset to the model
	 * to set stack of commands of this wizard.
	 * 
	 * The user will need to invoke the commands queue in order to actually
