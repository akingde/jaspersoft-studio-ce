...
public class JSSTemplateTransferDropTargetListener extends TemplateTransferDropTargetListener {
	...
	/**
	 * Runnable used to run the commands generated by the drop inside an asynchronous UI thread.
	 * This is done because on windows if the drop command doesn't end (and it doesn't end if the
	 * command executed inside open a dialog) at OS level the drag and drop is disabled, even outside
	 * the application. To overcame this the command are executed inside a thread so the drop operation
	 * can end after the generation of the command and the run of the thread. The only thing to pay attention
	 * is that the code executed inside the thread must not access the current event, becuase since the drop
	 * operation is ended the event doesn't exist anymore
	 * 
	...
	protected class CommandRunnable implements Runnable{
		...
		/**
		 * A Lockable refresh node, it is used as lock when converting the command 
		 * into a JSSCompoundCommand. If this is null the command is executed normally
		 */
		 ...
		 * Create the runnable 
		 * 
		 * @param commandToExecute The command to execute, can't be null
		 * @param lockReferenceNode The lock node for the jss compound commands, can be null
		 * @param elementToSelect the element to select after the execution of the command, can be null
		 */
	...
	/**
	 * Search in the current event a node and use it to find  the current
	 * lockable node of the editor
	 * 
	...
	protected void handleDrop() {
		...
		if (movingDetails != null) {
			...
			final Object elementToSelect = getCreateRequest().getNewObject();
	...
	/**
	 * Convert a CompoundCommand into a JSSCompoundCommand, using the 
	 * passed node as lock for the refresh when the new JSSCompoundCommand
	 * is executed
	 * 
	 * @param cmd the CompoundCommand
	 * @param node the lock node for the new JSSCompoundCommand, if this is null it 
	...
	/**
	 * Select in the editor the edit part that has as model the 
	 * passed object 
	 * 
	 * @param model the object to select or null to don't select anything
	 */
