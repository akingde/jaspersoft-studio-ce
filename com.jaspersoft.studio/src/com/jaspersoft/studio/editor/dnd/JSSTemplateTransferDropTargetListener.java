...
/**
 * Custom transfer drop listener for DND operations that supports the
 * {@link DialogEnabledCommand} commands.
 * <p>
 * 
 * The code in the {@link #handleDrop()} method is similar to the one used in
 * {@link JDCreationTool#performCreation(int)}. We want to emulate the same
 * behavior.<br/>
 * A working example for this situation is the {@link ImageCreationDialog} that
 * is popup when creating an {@link JRDesignImage} element.
 * 
...
public class JSSTemplateTransferDropTargetListener extends AbstractTransferDropTargetListener {
	...
	/**
	 * The last edit part where the feedback is show. It is calculated not with
	 * getTargetEditPart but the method getContainer
	 */
	private EditPart feedBackContaienr = null;

	/**
	 * Runnable used to run the commands generated by the drop inside an
	 * asynchronous UI thread. This is done because on windows if the drop command
	 * doesn't end (and it doesn't end if the command executed inside open a dialog)
	 * at OS level the drag and drop is disabled, even outside the application. To
	 * overcame this the command are executed inside a thread so the drop operation
	 * can end after the generation of the command and the run of the thread. The
	 * only thing to pay attention is that the code executed inside the thread must
	 * not access the current event, becuase since the drop operation is ended the
	...
	protected class CommandRunnable implements Runnable {
		...
		/**
		 * A Lockable refresh node, it is used as lock when converting the command into
		 * a JSSCompoundCommand. If this is null the command is executed normally
		 */
		private ANode lockReferenceNode;

		/**
		 * The element created by the drop operation, so it can be selected after the
		 * operation is completed. If this is null nothing is selected
		 */
		 ...
		 *            the element to select after the execution of the command, can be
		 *            null
		 */
	...
	/**
	 * Get a compound command and a list of commands and add to the compound command
	...
	/**
	 * Check if at the report was added a Field and if it it was added into the
	 * detail band. In this case it can create a static text as title for the field,
	 * and pace it into the column header or in the detail (if in the column header
	 * there isn't enough space). The Static Text creation behavior is specified by
	 * a properties read from the preference store
	 * 
	 * @param previusCommand
	 *            the command that create the filed, must be a CompundCommand that
	 *            contains the create element command
	 */
	private void createLabelForField(Command previusCommand) {
		...
		if (target instanceof CreateRequest) {
			...
			if (container instanceof BandEditPart && ((MBand) container.getModel()).getBandType() == BandTypeEnum.DETAIL
				...
				// It is a creation request, into the detail, with one or more commands
				// encapsulated into a compound one
				...
				for (Object command : compCommand.getCommands()) {
					...
					} else if (command instanceof CreateE4ObjectCommand
				...
				for (CreateE4ObjectCommand creatElementC : creationCommands) {
					...
					JRDesignStaticText newTextElement = (JRDesignStaticText) newText
							.createJRElement(band.getJasperDesign());
					String labelText = field.getDisplayText();
					String plabel = field.getValue().getPropertiesMap().getProperty(DataQueryAdapters.FIELD_LABEL);
					if (!Misc.isNullOrEmpty(plabel))
						labelText = plabel;

					Boolean useDescription = field.getJasperConfiguration()
							.getPropertyBoolean(DesignerPreferencePage.P_USE_FIELD_DESCRIPTION, false);
					if (useDescription) {
					...
					if (placeinTheHedaer) {
						// There is enough space in the Column header, the static text will be placed
						// into it
						...
						// There isn't enough space in the Column header, the static text will be placed
						// into the detail if only one
						// field is dragged
						int x = creatElementC.getLocation().x - band.getBounds().x;
						int y = creatElementC.getLocation().y - band.getBounds().y;
						int actualWidth = creatElementC.getLocation().width != -1 ? creatElementC.getLocation().width
					...
					if (location != null) {
						...
						if (dragBehavior.equals(DesignerPreferencePage.BEHAVIOR_ASK_EVERYTIME)) {
							...
							MessageDialogWithToggle question = MessageDialogWithToggle.open(
									MessageDialogWithToggle.QUESTION, UIUtils.getShell(),
									Messages.JSSTemplateTransferDropTargetListener_createLabelTitle, dragMessage, null,
				...
				// Add to the CompoundCommand all the command for the label, if they are
				// requested
	 ...
	 * 
	 * @return the container of the actual target edit part, if the target edit part
	 *         is not a container, or the target edit part itself otherwise
	 */
	private EditPart getContainer() {
		EditPart target = getTargetEditPart();
		if (!(target instanceof IContainer)) {
			ANode parentModel = ((ANode) target.getModel()).getParent();
			// This use the model for the search because every EditPart in the report has
			// the same father.
	...
	/**
	 * Show the feedback where the element will be created and not on the the drop
	 * target element
	 */
	@Override
	...
	/**
	 * Check if the user is dragging a detail band to move it before or after
	 * another detail band. In this case it return the command to do this operation,
	 * otherwise null
	 * 
	 * @return command to move the detail band or null if the user is not moving the
	 *         band
	 */
	...
	/**
	 * Search in the current event a node and use it to find the current lockable
	 * node of the editor
	 * 
	...
	/**
	 * Convert a CompoundCommand into a JSSCompoundCommand, using the passed node as
	 * lock for the refresh when the new JSSCompoundCommand is executed
	 * 
	 * @param cmd
	 *            the CompoundCommand
	 * @param node
	 *            the lock node for the new JSSCompoundCommand, if this is null it
	 *            will act like a standard CompoundCommand
	 */
				...
				if (size() > 0) {
					...
					for (int i = 0; i < size(); i++) {
						...
						if (cmd instanceof CreateElementCommand) {
							...
							createCnd.execute();
							// if one command is cancelled during the execution even the following are
							// skipped
