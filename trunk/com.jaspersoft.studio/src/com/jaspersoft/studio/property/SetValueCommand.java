...
public class SetValueCommand extends Command {
	...
	public void execute() {
		/*
		 * Fix for Bug# 54250 IPropertySource.isPropertySet(String) returns false
		 * both when there is no default value, and when there is a default value
		 * and the property is set to that value. To correctly determine if a reset
		 * should be done during undo, we compare the return value of
		 * isPropertySet(String) before and after setPropertyValue(...) is invoked.
		 * If they are different (it must have been false before and true after --
		...
		if (getTarget() instanceof IPropertySource2)
			resetOnUndo = !wasPropertySet && ((IPropertySource2) getTarget()).isPropertyResettable(propertyName);
		else
