...
import com.jaspersoft.studio.editor.action.CreateElementAction;
...
public class JDTreeContainerEditPolicy extends TreeContainerEditPolicy {
	...
	protected Command createCreateCommand(ANode child, int index) {
		return OutlineTreeEditPartFactory.getCreateCommand((ANode) getHost().getModel(), child, new Point(0, 0), index);
	}
	...
	protected Command getCreateCommand(CreateRequest request) {
		...
		if (request.getNewObject() instanceof ANode) {
			...
			action.dropInto(getHost().getModel(), new Point(), index);
			action.run();
